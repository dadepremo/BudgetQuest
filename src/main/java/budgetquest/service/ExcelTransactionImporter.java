...
public class ExcelTransactionImporter {
    ...
    public static Map<Category, Transaction> importFromExcel(File file, User user) {
        ...
        try (FileInputStream fis = new FileInputStream(file);
            ...
            for (Row row : sheet) {
                if (!dataStarted) {
                    String firstCellValue = getCellStringValue(row, 0).trim();
                    logger.info("Row " + row.getRowNum() + " - first cell: '" + firstCellValue + "'");

                    if ("Tipologia".equalsIgnoreCase(firstCellValue.trim())) {
                        dataStarted = true;
                ...
                if (rawAmount == null) {
                    String amountStr = getCellStringValue(row, 7).trim();
                ...
                String description = getCellStringValue(row, 6).trim();
                ...
                if ("uscite".equalsIgnoreCase(firstCellValue) || "expense".equalsIgnoreCase(firstCellValue)) {
                    for (Category c : categories) {
                        if (c.getName().equalsIgnoreCase(getCellStringValue(row, 2).trim()) && c.getType().equals("expense")) {
                    ...
                    amount = BigDecimal.valueOf(rawAmount);
                    for (Category c : categories) {
                        if (c.getName().equalsIgnoreCase(getCellStringValue(row, 2).trim()) && c.getType().equals("income")) {
                            category.setName(c.getName());
                            category.setId(c.getId());
                            break;
                        }
                ...
                // Prepare transaction
                String name = getCellStringValue(row, 5).trim();
